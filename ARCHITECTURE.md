...
| `figma_diagram` | Draw an activity diagram (with or without swimlanes), a sequence diagram, a state machine, an ERD, a userflow, a sitemap, a use case diagram, a journey map or a persona set from a model the agent derived from the spec, and report the holes in that model. Nine kinds behind one `type`, because a tool description is paid for in every session. Takes the arrays or the compact `text` form, draws a placed set from `diagrams: [...]`, and redraws a frame in place from `update` + `patch`. Each draw is cross-checked against the other diagrams on its page. |
| `figma_docs` | On-demand docs: `rules` \| `layout` \| `api` \| `tokens` \| `icons` \| `recipes` \| `style` \| `userflow` \| `activity` \| `erd` \| `sequence` \| `sitemap` \| `state` \| `persona` \| `journey` \| `usecase`. |
...
The other eight kinds share that split and most of that code. `src/shared/diagram/` holds what every diagram needs (text metrics, the (along, cross) axis projection, the palette, arrow emission, graph checks); each kind adds only what is genuinely its own — lane-constrained placement for an activity, columns and rows for a sequence, a stage×lane grid with an emotion curve for a journey, a measured card grid for a persona — and `src/plugin/diagram-reflow.ts` is the one door the live watcher and the `reflow_diagram` op come through, so adding a kind means adding it in one place rather than teaching every caller a new marker.

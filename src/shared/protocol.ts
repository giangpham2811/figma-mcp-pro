...
export const READ_OPERATIONS = [
  ...
  "get_page_model",
  // Component authoring — finding a component before you build a second one
  // is the whole point: a design system dies of near-duplicates, not of gaps.
  "find_component",
...
export const WRITE_OPERATIONS = [
  ...
  // which is why they are separated from ordinary node writes: a bad create()
  // makes one ugly frame, a bad create_variants() breaks every instance in
  // the file at once.
  ...
  // Instances: overrides, properties, and the two ways back to the main.
...
export const OP_TIMEOUTS: Partial<Record<Operation, number>> = {
  ...
  // Each of these walks the document looking for components or instances.
  find_component: 60_000,
  find_or_create_component: 60_000,
  match_main_values: 60_000,
  // combineAsVariants re-parents N components and re-links their instances.

...
const needsNodeId: Record<string, true> = {
  ...
  set_text_style: true,
  componentize: true,
  set_component_description: true,
  ...
  expose_nested_instance: true,
  detach_instance: true,
...
const OP_SCHEMAS: Partial<Record<Operation, z.ZodTypeAny>> = {
  ...
  // Component authoring
  //
  // The server owns identity (is there a target, is the name a string);
  // the plugin owns semantics (does this property type accept this value).
  // Splitting it the other way would put the Figma rulebook in two places
  ...
  find_or_create_component: z
    .object({
      ...
      // Absent `spec` means find-only; the plugin reports notFound instead
      // of inventing a component nobody described.
      spec: z.record(z.unknown()).optional(),
    })
    .passthrough(),
  componentize: z.object({ nodeId }).passthrough(),
  instantiate: z
    .object({
      componentId: nodeId.optional(),
      nodeId: nodeId.optional(),
    ...
    .passthrough()
    .refine((p) => p.componentId || p.nodeId || p.key, {
      message: "instantiate requires componentId (or nodeId), or a component key.",
    }),
  create_variants: z
    .object({
      // Two components is the minimum that can form a set; one is a rename.
      nodeIds: z.array(nodeId).min(2, "create_variants needs at least two components"),
      name: z.string().trim().min(1).optional(),
      parentId: nodeId.optional(),
    })
    .passthrough(),
  ...
  set_component_description: z
    .object({ nodeId, description: z.string() })
    .passthrough(),
  add_component_property: z
    .object({
      nodeId,
      name: z.string().trim().min(1),
      type: z
        ...
        .pipe(z.enum(["BOOLEAN", "TEXT", "INSTANCE_SWAP", "VARIANT"])),
      defaultValue: z.unknown().optional(),
    })
    .passthrough(),
  edit_component_property: z
    .object({
      nodeId,
      name: z.string().trim().min(1),
    ...
    .passthrough()
    .refine((p) => p.newName !== undefined || p.defaultValue !== undefined, {
      message: "edit_component_property needs newName or defaultValue.",
    }),
  delete_component_property: z
    .object({ nodeId, name: z.string().trim().min(1) })
    .passthrough(),

  get_instance_overrides: z.object({ nodeId }).passthrough(),
  set_instance_overrides: z
    .object({
      // One id or many: "make these twelve match that one" is the reason
      // this op exists, so the many case is not an afterthought.
      nodeId: nodeId.optional(),
      nodeIds: z.array(nodeId).min(1).optional(),
      source: z.record(z.unknown()).optional(),
      sourceInstanceId: nodeId.optional(),
    })
    .passthrough()
    .refine((p) => p.nodeId || (p.nodeIds && p.nodeIds.length > 0), {
      message: "set_instance_overrides requires nodeId or nodeIds.",
    })
    .refine((p) => p.source || p.sourceInstanceId, {
      message: "set_instance_overrides requires source (from get_instance_overrides) or sourceInstanceId.",
    }),
  set_instance_properties: z
    .object({ nodeId, props: z.record(z.unknown()) })
    .passthrough(),
  expose_nested_instance: z
    .object({ nodeId, exposed: z.boolean().optional() })
    .passthrough(),
